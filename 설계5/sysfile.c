...
sys_open(void)
{
  ...
  if(omode & O_CREATE){
    //20182586 김동해
    //open할때 O_CREATE 라면 T_CS로 create하도록 설정 
    if(omode & O_CS){
			if((ip = create(path, T_CS, 0, 0)) == 0){
        end_op();
				return -1;
      }
    }
...
sys_printinfo(void){
  ...
  filestat(file, &FileInfo);
	...
	if(FileInfo.type == 2){
		...
		//file에서 inode접근하여 길이 추출 후 출력
		for(int i=0; i<12;i++){
			if(!(file->ip->addrs[i] & 255))
				break;
			cprintf("[%d] %d\n",i,(file->ip->addrs[i]));
		}
		cprintf("\n");
	...
	//type==4 이면 T_CS
		...
		for(int i=0; i<12;i++){
			if(!(file->ip->addrs[i] & 255))
				break;
			cprintf("[%d] %d (num: %d, length: %d)\n",i, file->ip->addrs[i] ,(file->ip->addrs[i] >> 8), (file->ip->addrs[i] & 255));
		}
		cprintf("\n");
	}
  return 0;
